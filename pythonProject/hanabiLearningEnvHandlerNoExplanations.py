...
def interpret_move(move, hand):
    ...
    if "Play" in move:
        ...
        color = colors[card[0]]
        rank = ranks[card[1]]
        return f"giocare la carta {color} {rank}"
        ...
        color = colors[card[0]]
        rank = ranks[card[1]]
        return f"scartare la carta {color} {rank}"

    elif "rank" in move:
        rank = move.split()[-1]
        return f"suggerirti il rank {rank}"

    elif "color" in move:
        color = move.split()[-1]
        return f"suggerirti il colore {colors[color]}"
...
class HanabiHandler:
    def __init__(self, approachChoice, approachChoiceToM1, approachChoiceToM2,
        ...
        while not state.is_terminal():
            ...
            if self.gameChoice == "ToM":
                ...
                if currPlayer == 0:
                    ...
                    explanation = "Ho scelto di " + interpret_move(str(move)[1:-1], actualPlayerCards)
