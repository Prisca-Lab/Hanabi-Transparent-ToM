...
""" Max accuracy: 52.95% - Decision Tree per predire la migliore carta da scartare 
    nel caso in cui si esegua l'azione DISCARD """
""" L'accuratezza è relativa, nel senso che talvolta si può scartare la carta di index 0 ad esempio 
    ma anche quella di index 3, perché magari sono la stessa carta, oppure sono entrambe non giocabili 
    sui due fireworks red e yellow. """

# Leggiamo il dataset dal file CSV
df = pd.read_csv('../fullDatasetMDP.csv')
# Rimuovi le righe del dataset che contengono azioni diverse da DISCARD
df = df[(df['Best action'] == 'DISCARD')]
...
sorted_importances.to_csv('../../../models/decision_trees_models_MDP/'
                          'importances/decision_tree_discard_full_dataset_importances.txt', sep=' ', index=False)

# Salva il modello
joblib.dump(best_model, '../../../models/decision_trees_models_MDP/decision_tree_discard_full_dataset.joblib')
