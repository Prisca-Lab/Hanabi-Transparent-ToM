...
""" Max accuracy: 82.86% - Predizione del rank delle carte da suggerire """
...
data = data[(data['Best action'] == 'REVEAL_RANK')]

# Rimuoviamo i casi in cui viene dato un REVEAL_RANK ma con 0 info tokens, che sono casi errati
data = data[(data['Remaining info tokens'] != 0) | (data['Best action'] != 'REVEAL_RANK')]
...
data = pd.concat([data.drop(columns=['P2 Cards']),
                 ...
                 axis=1)
data = pd.concat([data.drop(columns=['Actual P2 Cards']),
                 data['Actual P2 Cards'].apply(encode_cards).apply(pd.Series).fillna(0).
...
sorted_importances.to_csv('../../../models/decision_trees_models_MDP/'
                          'importances/decision_tree_rank_full_dataset_importances.txt', sep=' ', index=False)

# Salva il modello
joblib.dump(best_model, '../../../models/decision_trees_models_MDP/decision_tree_rank_full_dataset.joblib')
